...
class NaiveBayes implements Estimator, Learner, Online, Probabilistic, Persistable
{
    ...
    {
        //TODO add parameter for WCNB (whether to do weights normalization or not)
        $logPriors = [];
    ...
    public function partial(Dataset $dataset) : void
    {
        ...
        foreach ($dataset->stratify() as $class => $stratum) {
            ...
            foreach ($stratum->features() as $column => $values) {
                ...
                //TODO compute negative count here
                $total = array_sum($columnCounts) + (count($columnCounts) * $this->smoothing);

                $probs = [];

                //TODO compute negative count here (eq 6)
        ...
        //TODO if normalizing weights, normalize here (eq. 8)
